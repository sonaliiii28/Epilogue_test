-- COMBINED: Drop legacy schema + apply DOCX schema
--
-- Paste this whole file into Supabase Cloud: SQL Editor → New query → Run.
-- WARNING: This DROPS legacy tables (irreversible).
...
CREATE TABLE IF NOT EXISTS public.hospice_agency_list (
  hospice_agency_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  name text NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.care_space (
  ...
  hospice_agency_id uuid REFERENCES public.hospice_agency_list (hospice_agency_id),
  full_name text NOT NULL,
  date_of_birth date,
  gender text,
  preferred_language text,
